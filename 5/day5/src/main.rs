...
fn get_input() -> Input {
    ...
    let starts_ends = buf_reader.lines().map(|line| {
    ...
    Input { start: starts_ends.iter().map(|x| x.0).collect(), end: starts_ends.iter().map(|x| x.1).collect() }
}

fn part1(input : &Input) -> i32 {
    let width = input.start.iter().zip(input.end.iter()).fold(0, |max, elem| {
        std::cmp::max(max, std::cmp::max(elem.0.x, elem.1.x))
    }) + 1;

    let height = input.start.iter().zip(input.end.iter()).fold(0, |max, elem| {
    ...
    for elem in input.start.iter().zip(input.end.iter()) {
        ...
        if is_horizontal(&start, &end) {
            ...
            let x_start = if start.x > end.x {
                end.x
            }
            else {
                start.x
            };
            let x_end = if start.x > end.x {
                start.x
            }
            else {
                end.x
        ...
        if is_vertical(&start, &end) {
            let y_start = if start.y > end.y {
                end.y
            }
            else {
                start.y
            };
            let y_end = if start.y > end.y {
                start.y
            }
            else {
                end.y
...
fn part2(input : &Input) -> i32 {
    ...
    for elem in input.start.iter().zip(input.end.iter()) {
        ...
        if is_diagonal(&start, &end) {
            ...
            let end_x = if start.x < end.x { end.x + 1 } else { end.x - 1 };
            let end_y = if start.y < end.y { end.y + 1 } else { end.y - 1 };
            for (x, y) in range_step(start.x, end_x, x_direction).zip(range_step(start.y, end_y, y_direction)) {
