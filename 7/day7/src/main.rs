...
fn part1(input : &Vec::<i32>) -> i32 {
    ...
    for position in input {
        min_alignment = std::cmp::min(min_alignment, input.iter().fold(0, |total, p| {
            total + (p - position).abs()
        }))
    }
    min_alignment
}
...
fn part2(input : &Vec::<i32>) -> i32 {
    ...
    let triangle_number = |n : i32| -> i32 {
        (n * (n + 1)) / 2
    };

    for position in min_position..=max_position {
        min_alignment = std::cmp::min(min_alignment, input.iter().fold(0, |total, p| {
            total + triangle_number((p - position).abs())
        }))
    }
    min_alignment
}
