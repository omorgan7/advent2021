...
fn get_input() -> Input {
    ...
    let patterns = buf_reader.lines().map(|line| {
        let unwrapped = line.unwrap();
        let mut parts = unwrapped.split(" | ");
        parts.nth(0).unwrap().split(" ").map(|word| String::from(word)).collect()
    }).collect();

    let digits = buf_reader1.lines().map(|line| {
        let unwrapped = line.unwrap();
        let mut parts = unwrapped.split(" | ");
        parts.nth(1).unwrap().split(" ").map(|word| String::from(word)).collect()
    }).collect();

    Input { patterns: patterns, digits : digits }
}

fn part1(input : &Input) -> i32 {
    input.digits.iter().fold(0, | total, digits | {
        total + digits.iter()
            .fold(0, | t, d | t + (d.as_bytes().len() == 2 ||
                                   d.as_bytes().len() == 4 ||
                                   d.as_bytes().len() == 3 ||
    ...
    input.patterns
        ...
        .fold(0,
            |total, (patterns, digits)| {
                ...
                // |
                //  _
                let bottom_left = difference(&(one_digits.to_owned() + seven_digits + four_digits), eight_digits);
                ...
                while solved_digits.iter().any(|d| *d == 0xFF as char) {
                    for p in patterns.iter().filter(|p| p.as_bytes().len() == 5) {
                        ...
                        if two_solved == false {
                            ...
                            solved_digits[2] = subtract(&top_left_middle, &middle).unwrap().as_bytes()[0] as char;
                            solved_digits[1] = subtract(&two, &middle).unwrap().as_bytes()[0] as char;
                            solved_digits[4] = subtract(&top_bottom_right, &String::from(solved_digits[1])).unwrap().as_bytes()[0] as char;
                        ...
                        if five_solved == false {
                            ...
                            solved_digits[5] = subtract(&bottom_left, &test).unwrap().as_bytes()[0] as char;
                ...
                let decode : Vec::<String> = {
                    let mut tmp : [String; 10] = [
                        // 0
                        solved_digits.iter().enumerate().filter_map(|(i, d)| if i != 3 { Some(d) } else { None }).collect(),
                        // 1
                        solved_digits.iter().enumerate().filter_map(|(i, d)| if i == 1 || i == 4 { Some(d) } else { None }).collect(),
                        // 2
                        solved_digits.iter().enumerate().filter_map(|(i, d)| if i != 2 && i != 4 { Some(d) } else { None }).collect(),
                        // 3
                        solved_digits.iter().enumerate().filter_map(|(i, d)| if i != 2 && i != 5 { Some(d) } else { None }).collect(),
                        // 4
                        solved_digits.iter().enumerate().filter_map(|(i, d)| if i != 0 && i != 5 && i != 6 { Some(d) } else { None }).collect(),
                        // 5
                        solved_digits.iter().enumerate().filter_map(|(i, d)| if i != 1 && i != 5 { Some(d) } else { None }).collect(),
                        // 6
                        solved_digits.iter().enumerate().filter_map(|(i, d)| if i != 1 { Some(d) } else { None }).collect(),
                        // 7
                        solved_digits.iter().enumerate().filter_map(|(i, d)| if i == 1 || i == 4 || i == 0 { Some(d) } else { None }).collect(),
                        // 8
                        solved_digits.iter().enumerate().filter_map(|(_, d)| Some(d) ).collect(),
                        // 9
                        solved_digits.iter().enumerate().filter_map(|(i, d)| if i != 5 { Some(d) } else { None }).collect(),
                    ];
                    for d in &mut tmp {
                        let t0 = &d[..];
                ...
                for (index, digit) in digits.iter().rev().enumerate() {
                    let decoded = decode.iter().enumerate().find(|(_, d)| {
                        let mut t0 = &digit[..];
                        let mut chars: Vec<char> = t0.chars().collect();
                        chars.sort_by(|a, b| b.cmp(a));
