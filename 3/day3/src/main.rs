...
fn part1(input: &Vec<String>) -> u64 {
    ...
    let one_totals: Vec<usize> = (0..number_length)
        .into_iter()
        .map(|index| {
    ...
    let gamma = u64::from_str_radix(
        &one_totals
            .iter()
            .map(|count| if count > &half_total { '1' } else { '0' })
            .collect::<String>(),
        2,
    )
    .unwrap();
...
fn part2(input: &Vec<String>) -> u64 {
    ...
    for index in (0..input[0].len()) {
        let filter = |set: Vec<String>, zero: char, one: char| {
            ...
            let select = if set.len() == 1 {
                set[0].as_bytes()[index] as char
            } else if one_totals >= half_total {
                zero
            } else {
                one
            };
            ...
            set.iter()
                .filter(|line| line.as_bytes()[index] as char == select)
                .cloned()
                .collect()
        };
