...
fn get_input() -> Vec::<Node> {
    ...
    for line in buf_reader.lines() {
        ...
        if nodes.iter().find(|node| *node.name == first).is_none() {
            nodes.push(Node { name: first.clone(), is_big: is_uppercase(&first), connections: Vec::<usize>::new() });
            let first_index = nodes.len() - 1;
            
            if let Some((i, _)) = nodes.iter().enumerate().find(|(_, node)| *node.name == second) {
                nodes[first_index].connections.push(i);
                nodes[i].connections.push(first_index);
            }
            else {
                nodes.push(Node { name : second.clone(), is_big: is_uppercase(&second), connections: vec![first_index] });
            ...
            nodes.push(Node { name: second.clone(), is_big: is_uppercase(&second), connections: Vec::<usize>::new() });
            let second_index = nodes.len() - 1;
            
            if let Some((i, _)) = nodes.iter().enumerate().find(|(_, node)| *node.name == first) {
                nodes[second_index].connections.push(i);
                nodes[i].connections.push(second_index);
            }
            else {
                nodes.push(Node { name : first.clone(), is_big: is_uppercase(&first), connections: vec![second_index] });
                let first_index = nodes.len() - 1;
            ...
            let first_index = nodes.iter().enumerate().find(|(_, node)| *node.name == first).unwrap().0;
            let second_index = nodes.iter().enumerate().find(|(_, node)| *node.name == second).unwrap().0;
...
fn visit_p1(node: usize, end: usize, input: &Vec::<Node>, visited: &mut Vec::<usize>, end_count: &mut i32) {
...
fn visit_p2(start: usize, node: usize, end: usize, input: &Vec::<Node>, visited: &mut HashMap::<usize, usize>, end_count: &mut i32) {
...
fn part1(input : &Vec::<Node>) -> i32 {
    ...
    let start = input.iter().enumerate().find(|(_, node)| node.name == "start").unwrap().0;
    let end = input.iter().enumerate().find(|(_, node)| node.name == "end").unwrap().0;
